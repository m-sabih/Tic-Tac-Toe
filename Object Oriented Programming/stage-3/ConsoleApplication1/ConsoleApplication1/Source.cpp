	...
	string currentDateTime() 
{
	...
	void loadgame(ifstream &inf){
		...
		TicTacToe temp(size);
		for (int i=0;i<size;i++){
		inf.read(reinterpret_cast<char*>(&temp), sizeof(int)*size);}
		inf.read(reinterpret_cast<char*>(&size1),sizeof(int));
		inf.read(reinterpret_cast<char*>(&time1),sizeof(string));
		while(!inf.eof()){
			cout<<id<<'\t'<<time1<<endl;
			for (int i=0;i<size;i++){
				inf.read(reinterpret_cast<char*>(&temp), sizeof(int)*size);}
	...
	void deletegame(ifstream &inf1, int delchoice){
		if (!inf1) {
			cout << "File is not Opened." << endl;
		}
		int id2=1;
		...
		for (int i=0;i<size;i++){
			inf1.read(reinterpret_cast<char*>(&temp), sizeof(int)*size);}
		inf1.read(reinterpret_cast<char*>(&size2),sizeof(int));
		inf1.read(reinterpret_cast<char*>(&time2),sizeof(string));
		while(!inf1.eof()){
			if (delchoice!=id2){
				for (int i=0;i<size;i++){
	...
	TicTacToe selectgame(ifstream &inf2, int levchoice){
		if (!inf2) {
			cout << "File is not Opened." << endl;
		}
		int id3=1;
		...
		TicTacToe temp(size);
		for (int i=0;i<size;i++){
			inf2.read(reinterpret_cast<char*>(&temp), sizeof(int)*size);
		}
		inf2.read(reinterpret_cast<char*>(&size3),sizeof(int));
		inf2.read(reinterpret_cast<char*>(&time3),sizeof(string));
		while(!inf2.eof()){
			if (levchoice==id3){
				return temp;
				count=1;
...
int main()
{
	...
	do 
	{
		char choicec;			//to check char
		int choice_;			//to input choice
		int choices;
		do
		{
			...
			cin>>choices;
			if (choices==1){
				fstream inf("game.dat", ios::binary | ios::in | ios::app);
			}
			else if (choices==2){
			}
			if (choices==3){
			...
			while (true)
			{
				...
				cin>>gchoice;
				if (gchoice=='s' || gchoice=='S'){
				ofstream outf("game.txt", ios::app);
				}
